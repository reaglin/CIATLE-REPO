...
.DESCRIPTION
  Finds the newest claude*.zip in Downloads, validates the contents look like
  curriculum-guide JSONs, extracts them into .\drafts\, runs queue.py reconcile,
  and prints a summary of what's now ready to push.

  Run from: C:\Users\ronal\source\repos\CIATLE-REPO\Tools

.PARAMETER ZipPath
  Explicit path to a ZIP file. Default: newest claude*.zip in Downloads.
...
.PARAMETER DownloadsDir
  Override the Downloads directory. Default: $env:USERPROFILE\Downloads.

.EXAMPLE
  .\import_drafts.ps1
  # Finds newest claude*.zip in Downloads, imports, reconciles, deletes ZIP.

.EXAMPLE
  .\import_drafts.ps1 -ZipPath "C:\some\other\path.zip" -KeepZip
  # Imports a specific ZIP and keeps it.

.EXAMPLE
  .\import_drafts.ps1 -Force
  # Overwrites any existing draft files without prompting.
#>
...
$QueueScript = Join-Path $ToolsDir 'queue.py'

# Ensure we're running from the right place
if (-not (Test-Path $QueueScript)) {
    Write-Error "queue.py not found in $ToolsDir. Run this script from the Tools directory."
    exit 1
}
...
if (-not $ZipPath) {
    ...
    $candidates = Get-ChildItem -Path $DownloadsDir -Filter 'claude*.zip' -File |
                  Sort-Object LastWriteTime -Descending
    if (-not $candidates) {
        Write-Error "No claude*.zip files found in $DownloadsDir.`nDownload the batch from chat first, or pass -ZipPath."
        exit 1
    }
    $ZipPath = $candidates[0].FullName
    Write-Host "Found ZIP: $ZipPath" -ForegroundColor Cyan
    if ($candidates.Count -gt 1) {
        Write-Host "  ($($candidates.Count) claude*.zip files in Downloads — using newest)" -ForegroundColor DarkGray
    }
} else {
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Warning "queue.py reconcile returned exit code $LASTEXITCODE"
    }
